...
% Last Modified by GUIDE v2.5 06-Nov-2016 06:16:24
...
% % %     return;
% % % end
if length(handles.obj.FileNumber) ==2
    filenumber = strcat(num2str(handles.obj.FileNumber(1)),'_',num2str(handles.obj.FileNumber(2)));
elseif length(handles.obj.FileNumber) ==1
    filenumber = num2str(handles.obj.FileNumber);
end

set(handles.txtLineNumber, 'String', filenumber);

fs = filesep;
filepath = pwd;    
fileName = strcat([filepath fs 'Seismics' fs 'Processed_files' fs filenumber ,'_picks.mat']);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% --- Executes on button press in Delete Pick button.
[X,T] = ginput;
X = MatchPicks(X,handles.TimePicks.RecXProf);
X = sort(X);
 
[RecXProf,IA] = setdiff(handles.TimePicks.RecXProf,X);
handles.TimePicks.RecXProf = RecXProf;
PickTime = handles.TimePicks.PickTime(IA);
ShotXLoc = handles.ShotLoc;
handles.TimePicks.PickTime =handles.TimePicks.PickTime(IA);


cla(handles.pick_plot)

%Overwrite the original picks file with this new data
...
if exist(Savepath,'dir')
    ...
    FilenameMat = [Savepath,fs,filenumber,'_picks.mat'];
    save(FilenameMat,'RecXProf','PickTime','ShotXLoc');
else
    mkdir (Savepath)
    FilenameMat = [Savepath,fs,filenumber,'_picks.mat'];
    save(FilenameMat,'RecXProf','PickTime','ShotXLoc');
...
% plot(handles.pick_plot,handles.TimePicks.RecXProf,handles.TimePicks.PickTime,'*g');
% xlabel('Geophone Position (m)');
% ylabel('First Arrival Time (ms)');
% hold on;
%  
%plot the shot
handles.hShot = plot(handles.pick_plot,handles.ShotLoc,0,'xr','MarkerSize',30); hold on;
% refreshdata(hObject,handles);

%******* setting the properties which will be used in other functions and
%        they set in openning as well
if ischar(handles.TimePicks.ShotXProf) == 1
    handles.ShotLoc = str2double(handles.TimePicks.ShotXProf); 
else
    handles.ShotLoc = handles.TimePicks.ShotXProf;
...
%     ylim([ minY 1]);
    ylimmin = str2double(sprintf('%.0f', -mxD));
    ylim([ ylimmin 1]);
    caxis([0 2500]);
...
%     daspect([1 1/VertExag 1]);
    ...
    plot(xshot,0.2,'xr','MarkerSize',30);
...
elseif handles.isStructurePlot == 0
        A = '';
end
handles.txt = A;
%**********************************************************
...
% --- Executes during object creation, after setting all properties.
function Delete_picks_CreateFcn(hObject, eventdata, handles)
% NOTE invisible in initial call
...
if exist(Savepath,'dir')
    %     save([Savepath fs,filename],'objFinal');
    Filenametxt = [Savepath,fs,filenumber,'_Structure_Parameters.txt'];
    
    Filenamejpg = [Savepath,fs,filenumber,'_Velocity_structure.jpg'];

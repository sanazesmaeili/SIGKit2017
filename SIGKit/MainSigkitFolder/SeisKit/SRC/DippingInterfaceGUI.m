...
% % catch
% %     msgString = {'There is no picking file for forward shot in the following directory: ' ...
% %              ,fileNameFwd};
% %     msgbox(msgString,'Warning','error');
% %     guidata(hObject, handles);
% % end
...
% % catch
% %     msgString = {'There is no picking file for reverse shot in the following directory: ' ...
% %              ,fileNameRvs};
% %     uiwait(msgbox(msgString,'Warning','error'));
% %     guidata(hObject, handles);
% % end
...
if length(handles.objFwd.FileNumber) ==2
    filenumberFwd = strcat(num2str(handles.objFwd.FileNumber(1)),'_',num2str(handles.objFwd.FileNumber(2)));
elseif length(handles.objFwd.FileNumber) ==1
    filenumberFwd = num2str(handles.objFwd.FileNumber);
end
if length(handles.objRvs.FileNumber) ==2
    filenumberRvs = strcat(num2str(handles.objRvs.FileNumber(1)),'_',num2str(handles.objRvs.FileNumber(2)));
elseif length(handles.objRvs.FileNumber) ==1
    filenumberRvs = num2str(handles.objRvs.FileNumber);
end

if exist(Savepath,'dir')

    Filenametxt = [Savepath,fs,filenumberFwd,'_',filenumberRvs,'_Structure_Parameters.txt'];
    
    Filenamepng = [Savepath,fs,filenumberFwd,'_',filenumberRvs,'_Velocity_structure.png'];
